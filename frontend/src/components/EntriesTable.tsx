...
import { payeeColorStyle } from "../lib/payeeColor";
...
export function EntriesTable({
  ...
  const shiftPressedOnMouseDownRef = useRef(false);
  const selectedSet = useMemo(() => new Set(selectedIds), [selectedIds]);
  const payeeById = useMemo(() => {
    const map = new Map<number, Payee>();
    for (const payee of payees) map.set(payee.id, payee);
    return map;
  }, [payees]);
  ...
  const selectedTotalsByCurrency = useMemo(
    () => {
      ...
      for (const entry of selectedEntries) {
        ...
        const currency = (entry.currency || "GBP").toUpperCase();
  ...
  const selectedTotalLabel = useMemo(() => {
    if (selectedTotalsByCurrency.size === 0) return "0.00";
    if (selectedTotalsByCurrency.size === 1) {
      const [currency, total] = selectedTotalsByCurrency.entries().next().value as [string, number];
      return formatAmount(total.toFixed(2), currency);
    }
    return [...selectedTotalsByCurrency.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([currency, total]) => `${currency} ${formatAmount(total.toFixed(2))}`)
      .join(" + ");
  }, [selectedTotalsByCurrency]);
  ...
  function toggleSelectionWithShift(
    id: number,
    index: number,
    checked: boolean,
    shiftKey: boolean
  ...
  function toggleSelectAll(checked: boolean) {
    if (checked) {
      ...
      setLastSelectedIndex(entries.length > 0 ? entries.length - 1 : null);
      return;
    }
    setSelectedIds([]);
    setLastSelectedIndex(null);
  ...
  return (
    <>
      ...
      <div className="bulk-actions-bar">
        ...
        <span className="hint">Selected total: {selectedTotalLabel}</span>
              ...
              <tr key={e.id}>
                <td>
                  <input
                    ...
                    onMouseDown={(ev) => {
                      shiftPressedOnMouseDownRef.current = ev.shiftKey;
                    }}
                    onChange={(ev) =>
                      toggleSelectionWithShift(
                        ...
                        rowIndex,
                        ev.target.checked,
                        shiftPressedOnMouseDownRef.current ||
                          Boolean(
                            (ev.nativeEvent as MouseEvent | undefined)?.shiftKey
                          )
                      )
                    }
                  />
                ...
                <td>
                  <PayeeSelect
                    ...
                    className={`payee-select-colored${e.payeeId == null ? " unassigned" : ""}`}
                    style={
                      e.payeeId == null
                        ? undefined
                        : payeeColorStyle(e.payeeId, payeeById.get(e.payeeId)?.color)
                    }
