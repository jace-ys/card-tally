...
fn normalize_payee_color(input: Option<&str>) -> AppResult<Option<String>> {
    ...
    if !is_hex {
        return Err(AppError::BadRequest(
            "invalid color; use #RRGGBB".to_string(),
        ));
    }
...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/summary-by-payee", get(all_statements_summary_by_payee))
...
async fn post_import(
    ...
    if lines.is_empty() {
        return Err(AppError::BadRequest(
            "no new entries found; all rows already exist".into(),
        ));
    }
...
async fn statement_entries(
    ...
    let sql = format!(
        "SELECT id, statement_id, txn_date, merchant_raw, merchant_key, amount_cents, amount_currency, payee_id, lifecycle \
...
async fn patch_entry(
    ...
    let before: Option<EntryRow> = sqlx::query_as(
        "SELECT id, statement_id, txn_date, merchant_raw, merchant_key, amount_cents, amount_currency, payee_id, lifecycle FROM entries WHERE id = ?",
    )
    .bind(id)
...
async fn quick_assign(
    ...
    let candidate: Option<EntryRow> = sqlx::query_as(
        r#"SELECT id, statement_id, txn_date, merchant_raw, merchant_key, amount_cents, amount_currency, payee_id, lifecycle
           FROM entries
...
async fn statement_summary_by_payee(
    ...
    let sql = format!(
        r#"SELECT e.payee_id AS payee_id,
           ...
           GROUP BY e.payee_id, p.name, e.amount_currency
           ORDER BY payee_name, e.amount_currency"#
    );
...
async fn all_statements_summary_by_payee(
    State(state): State<AppState>,
    Query(q): Query<SummaryQuery>,
) -> AppResult<Json<StatementPayeeSummaryDto>> {
    let archived_only = q.status.as_deref() == Some("paid_archived");
    ...
    let raw_rows: Vec<SummaryAggRow> = sqlx::query_as(&sql).fetch_all(&state.pool).await?;
    ...
    Ok(Json(StatementPayeeSummaryDto {
        statement_id: 0,
        statement_name: "All statements".to_string(),
        rows,
    }))
}
...
async fn list_payees(State(state): State<AppState>) -> AppResult<Json<Vec<PayeeDto>>> {
    let rows: Vec<PayeeRow> = sqlx::query_as(
        "SELECT id, name, shortcut_slot, sort_order, color FROM payees ORDER BY sort_order ASC, name ASC",
    )
    .fetch_all(&state.pool)
    .await?;
...
async fn create_payee(
    ...
    let color = normalize_payee_color(body.color.as_deref())?;

    let res = sqlx::query_as::<_, PayeeRow>(
        "INSERT INTO payees (name, shortcut_slot, sort_order, color) VALUES (?, ?, ?, ?) RETURNING id, name, shortcut_slot, sort_order, color",
    )
    .bind(&name)
...
async fn update_payee(
    ...
    let existing: Option<PayeeRow> = sqlx::query_as(
        "SELECT id, name, shortcut_slot, sort_order, color FROM payees WHERE id = ?",
    )
    .bind(id)
    .fetch_optional(&state.pool)
    .await?;
    ...
    if let Some(c) = body.color {
        row.color = normalize_payee_color(c.as_deref())?;
    }

    let res = sqlx::query_as::<_, PayeeRow>(
        "UPDATE payees SET name = ?, shortcut_slot = ?, sort_order = ?, color = ? WHERE id = ? RETURNING id, name, shortcut_slot, sort_order, color",
    )
    .bind(&row.name)
