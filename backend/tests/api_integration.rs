...
#[tokio::test]
async fn import_prunes_entries_already_seen_before() {
    let (addr, _dir, _pool) = spawn_app().await;
    let base = format!("http://{}/api", addr);
    let client = reqwest::Client::new();
...
#[tokio::test]
async fn summary_by_payee_across_all_statements() {
    let (addr, _dir, _pool) = spawn_app().await;
    let base = format!("http://{}/api", addr);
    let client = reqwest::Client::new();

    let amex = include_bytes!("fixtures/sample_amex.csv");
    let part = multipart::Part::bytes(amex.to_vec())
        .file_name("Amex.csv")
        .mime_str("text/csv")
        .unwrap();
    let form = multipart::Form::new().part("file", part);
    ...
    let yonder = include_bytes!("fixtures/sample_yonder.csv");
    let part = multipart::Part::bytes(yonder.to_vec())
        .file_name("Yonder.csv")
        .mime_str("text/csv")
        .unwrap();
    let form = multipart::Form::new().part("file", part);
    ...
    let summary: Value = client
        .get(format!("{base}/summary-by-payee"))
        .send()
        .await
        .unwrap()
        .json()
        .await
        .unwrap();
    assert_eq!(summary["statementName"], "All statements");
    assert!(summary["rows"].as_array().unwrap().len() >= 1);
}
